...

{
    ...
    void Update()
    {
        ...
        if (changePosition(oldPosVec, StartPos.transform.position, Desitination.transform.GetChild(RoomNr).transform.position))
        {
            grid.destroyNavObj();
            FindPath(StartPos.transform.position, Desitination.transform.GetChild(RoomNr).transform.position);
            grid.drawPathObject();
        }
